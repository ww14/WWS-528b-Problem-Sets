...
/*Method 1
sort wealth
pshare histogram, ylabel(0(0.05)2, angle(hor)) ti(Wealth distribution) ///
	noci  prange(0 95)

sort lnwealth
pshare estimate lnwealth, p(0(5)95) density
pshare histogram, ylabel(0(0.25)1.5, angle(hor)) ti(Log Wealth distribution) ///
	noci prange(0 100)
pause*/

*Method 2, Population Share
sort wealth 
gen id=_n if wealth !=.
sum id
...
//why does r1 work here and not r`i'?
...
*Method 2, construction needed for section 7 and 8
gen popsharei = .
...
tab popsharei, sum(wealthi) means 	
...
pshare histogram,  ylabel(0(0.1)0.5, angle(hor)) ti(Wealth distribution) noci

/*
sort wealthwt
pshare estimate wealthwt, nquantiles(100)
pshare histogram,  ylabel(0(0.01)0.1, angle(hor)) yscale(r(0 0.1)) ti(Wealth distribution) noci
*/
...
lorenz graph, aspectratio(1) xlabel(, grid) lcolor("22 150 210") noci

*Chris' alternative
line wealthshare popshare if inrange(popshare, 0, 100), lcolor("22 150 210") aspectratio(1) || (function y=x, range(0 100) lcolor(grey))
...
/******************************************************************************/
/*
*Model 1. This is incorrect, b/c it gives distribution conditional on being self-employed
sort wealthwt
pshare estimate wealthwt, p(50 90 99) over(selfemp)
pshare estimate wealthwt if X4106==2 | X4706==2, p(99.9 100)

*Model 2, with self-employed spouse
sort wealthwt
pshare estimate wealthwt if X4106==2, p(50 90 99) 
pshare estimate wealthwt if X4106==2, p(99.9 100)

*appears to make a difference for bottom 50 but not a huge change for the rest
*/

gen selfemp=.
replace selfemp=0 if popshare!=.
replace selfemp=1 if X4106>1 | X4706>1
//I disagree with Chris here. It needs to be zero not null to get the approriate average


gen selfempi=.
label var selfempi "fraction of pop who are 'entreprenuers'"
...
graph bar (mean) selfempi, over(popsharei) ysc(r(0 1))
...
*Model 1 
gen white_1i=.
label var white_1i "fraction of pop who are white"

forval i=1/5{
	sum white_1 if popsharei==`i'
	di `r(mean)'
	replace white_1i=`r(mean)' if popsharei==`i'
}

graph bar (mean) white_1i, over(popsharei) ysc(r(0 1))

pause

*Model 2 
gen white_2i=.
label var white_2i "fraction of pop who are white"

forval i=1/5{
	sum white_2 if popsharei==`i'
	di `r(mean)'
	replace white_2i=`r(mean)' if popsharei==`i'
	}

graph bar (mean) white_2i, over(popsharei) ysc(r(0 1))

/*
*Model 1
sort wealthwt
pshare estimate wealthwt if X6809==1 , p(50 90 99) 
pshare estimate wealthwt if X4106==1, p(99.9 100)

*Model 2: If self-identify into multiple races, pick up white in all available self-identify categories
sort wealthwt
pshare estimate wealthwt if X6809==1 | X6810==1 , p(50 90 99) 
pshare estimate wealthwt if X6809==1 | X6810==1, p(99.9 100)
